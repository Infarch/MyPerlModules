...
our $verbose_categories = 0;
our $verbose_products = 0;
...
sub test_pics {
	# check whether there are no pictures with zero length
	my @pics = Product->new()->newProductPicture()->selectAll($dbh);
	print scalar @pics, " pictures\n";
	foreach my $picture (@pics){
		my $pic_id = $picture->ID;
		my $from1 = "$src_product_pictures/$pic_id.jpg";
		my $from2 = "$src_product_pictures/$pic_id-enl.jpg";
		my $s1 = -s $from1;
		my $s2 = -s $from2;
		unless($s1 && $s2){
			print $picture->get('URL'), "\n";
			$picture->markFailed();
			$picture->update($dbh);
		}
	}
...
sub process_category {
	...
	# 'catalog.1.' must be 'catalog.2.' for the next data source (parsing)
	
	my $pic_file = 'catalog.1.'.$cat_id.'.jpg';
	...
	unless(-e $path_name){
		...
		if(@piclist > 0){
			...
			my $pic_id = $picture->ID;
			die "no picture" unless -e "$src_product_pictures/$pic_id.jpg";
			category_thumb("$src_product_pictures/$pic_id.jpg", $path_name);
