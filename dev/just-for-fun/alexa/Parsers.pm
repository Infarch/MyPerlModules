...
sub get_categories {
	...
	if($level==0){
		$node_clause = q{//div[@id='catList']//div[@class='categories ']//a | //div[@class='categories top']//a};
...
sub get_products {
	my ($tree) = @_;
	my @list;
	my @nodes = $tree->findnodes( q{//li[@class='site-listing']/div[@class='desc-container']} ); # <------------ write clause here
	foreach my $node(@nodes){
		my %h;
		$h{Name} = $node->findvalue( q{./h2/a} ); # <------------ write clause here
		$h{URL} = $node->findvalue( q{./h2/a/@href} ); # <------------ write clause here
		$h{ShortDescription} = $node->findvalue( q{./div[@class='description']} ); # <------------ write clause here
		
		$h{ShortDescription} =~ s/\.\.\. More//;
...
sub get_product_info {
	...
	$h{FullDescription} = $tree->findvalue( q{/html/body/div[2]/div/div[3]/div/div/div/div/h1/a} ); # <------------ write clause here
...
sub get_next_page {
	...
	return $tree->findvalue( q{//div[@class='pageNav']//a[@class='next']/@href} );
