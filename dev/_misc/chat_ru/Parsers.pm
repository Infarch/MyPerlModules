...
sub get_products {	
	...
	$content =~ s/\r|\n|\t/ /g;
	$content =~ s/\s{2,}/ /g;
	$content =~ /<!-- story about founded information -->(.*?)<!-- end oF story about founded information-->/;
	
	$content = $1;
	
	my @list;

	while ($content =~ /<td bgcolor='(#f5f5eb|#ffffff)'>(.*?)<br><br>\s<\/td>\s<\/tr>/g){
		my $node = $2;
		my %h;
		if($node =~ /<font size=4><a target='_blank' href='[^']+'>(.*?)<\/a>/) #'
		{
			$h{Name} = $1;
		}
		if($node =~ /<br><font size='-1'>\[<a target='_blank' href='[^']+'>(.*?)<\/a>]<\/font>/) #'
		{
			$h{URL} = $1;
		}
		push @list, \%h;
