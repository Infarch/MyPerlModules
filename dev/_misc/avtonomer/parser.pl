...
sub check_root {
	...
	unless($root->checkExistence($dbh)){
		...
		# insert several pre-defined categories
		my @cl = (
			"http://avto-nomer.ru/gallery.php?&region=78",
			"http://avto-nomer.ru/gallery.php?region=98",
			"http://avto-nomer.ru/gallery.php?&region=178",
			#"http://avto-nomer.ru/gallery.php?region=198"
		);
		...
		foreach my $cat(@cl){
			...
			$cat_obj->set("URL", $cat);
			$cat_obj->set("Level", 1);
			$cat_obj->set("Category_ID", $root->ID);
			$cat_obj->set("Name", $1);
			$cat_obj->insert($dbh);
		}
